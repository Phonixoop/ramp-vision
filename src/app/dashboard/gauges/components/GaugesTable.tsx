...
import { useGauges } from "../context";
import { GaugesTableProps } from "../types";
import {
  defaultProjectTypes,
        ...
        <div className="mx-auto flex w-11/12 items-center justify-between gap-5 py-5 md:flex-row-reverse md:items-start">
          <div className="flex w-full flex-col items-center justify-center gap-5">
            ...
            {getCitiesWithPerformance.isLoading ? (
              <>
                {Array.from(
                  Array(getInitialFilters?.data?.Cities?.length ?? 31).keys(),
                ).map((a, i) => {
                  return (
                    <div
                      key={i}
                ...
                <>
                  ...
                  <div className="flex min-w-[15rem] max-w-sm flex-col items-center justify-center gap-3 rounded-xl bg-secondary p-2">
                    ...
                    <SelectControlled
                      ...
                      title={"نوع پروژه"}
                      list={
                        getInitialFilters?.data?.projectTypes?.map(
                          (a: any) => a.ProjectType,
                        ) ?? []
                      }
